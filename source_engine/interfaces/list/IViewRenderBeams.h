...
struct Beam_t
{
	...
	virtual const vector3d_t<float>& GetRenderOrigin() = 0;
	virtual const vector3d_t<float>& GetRenderAngles() = 0;
	virtual const matrix3x4_t& RenderableToWorldTransform() = 0;
	virtual void			GetRenderBounds(vector3d_t<float>& vecMins, vector3d_t<float>& vecMaxs) = 0;
	...
	vector3d_t<float>			vecMins;
	vector3d_t<float>			vecMaxs;
	...
	int				nAttachments;
	vector3d_t<float>			vecAttachment[MAX_BEAM_ENTS];
	vector3d_t<float>			vecDelta;
...
struct BeamInfo_t
{
	...
	// Points
	vector3d_t<float>			vecStart;
	vector3d_t<float>			vecEnd;
	...
	// Rings
	vector3d_t<float>			vecCenter;
	float			flStartRadius;
	float			flEndRadius;
};

struct IViewRenderBeams
{
private:
	enum eIndices
	{
		DRAWBEAM = 4,
		CREATEBEAMRINGPOINT = 12
	};
public:
	...
	virtual void	DrawBeam(void* pBeam, const void* instance, void* pEntityBeamTraceFilter = nullptr) = 0;
	virtual void	DrawBeam(Beam_t* pBeam) = 0;

	virtual void	KillDeadBeams(void* pEntity) = 0;
	...
	virtual void	CreateBeamEntPoint(int iStartEntity, const vector3d_t<float>* pStart, int iEndEntity, const vector3d_t<float>* pEnd,
	...
	virtual void	CreateBeamPoints(vector3d_t<float>& vecStart, vector3d_t<float>& vecEnd, int nModelIndex, int iHaloIndex, float flHaloScale,
		float flLife, float flWidth, float flEndWidth, float flFadeLength, float flAmplitude,
		float flBrightness, float flSpeed, int iStartFrame,
		float flFrameRate, float r, float g, float b) = 0;
	...
	virtual void	CreateBeamRingPoint(const vector3d_t<float>& vecCenter, float flStartRadius, float flEndRadius, int nModelIndex, int iHaloIndex, float flHaloScale,
		float flLife, float flWidth, float flEndWidth, float flFadeLength, float flAmplitude,
		float flBrightness, float flSpeed, int iStartFrame,
		float flFrameRate, float r, float g, float b, int iFlags = 0) = 0;
	virtual void	CreateBeamCirclePoints(int iType, vector3d_t<float>& vecStart, vector3d_t<float>& vecEnd,
