...
namespace FGUI
{
  ...
  void CTabPanel::Geometry(FGUI::WIDGET_STATUS status)
  {
    ...
    for (std::size_t i = 0; i < m_prgpTabButtons.size(); i++)
    {
      ...
      {
        m_iEntrySpacing = m_dmSize.m_iWidth;
        ...
        if (m_prgpTabButtons.at(i) == m_prgpTabButtons.front())
            FGUI::RENDER.Outline(arWidgetRegion.m_iLeft, arWidgetRegion.m_iTop, (arWidgetRegion.m_iRight * m_prgpTabButtons.size()) + 2, arWidgetRegion.m_iBottom, { 40, 40, 40 });

        if (m_ullSelectedEntry == i)
            FGUI::RENDER.Rectangle(arWidgetRegion.m_iLeft + 2, arWidgetRegion.m_iTop + 2, arWidgetRegion.m_iRight - 4, arWidgetRegion.m_iBottom - 4, { 22, 22, 22 });
        else
            FGUI::RENDER.Rectangle(arWidgetRegion.m_iLeft + 2, arWidgetRegion.m_iTop + 2, arWidgetRegion.m_iRight - 2, arWidgetRegion.m_iBottom - 4, { 30, 30, 30 });

        FGUI::RENDER.Text((arWidgetRegion.m_iLeft + (m_dmSize.m_iWidth / 2) - (FGUI::RENDER.GetTextSize(m_anyFont, m_prgpTabButtons[i]).m_iWidth / 2)), (arWidgetRegion.m_iTop + (m_dmSize.m_iHeight / 2) - (FGUI::RENDER.GetTextSize(m_anyFont, m_prgpTabButtons[i]).m_iHeight / 2)), m_anyFont, { 255, 255, 255 }, m_prgpTabButtons[i]);
      }
      else if (m_nStyle == static_cast<int>(TAB_STYLE::VERTICAL))
      {
        m_iEntrySpacing = m_dmSize.m_iHeight;
