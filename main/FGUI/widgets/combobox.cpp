...
namespace FGUI
{
  ...
  void CComboBox::Geometry(FGUI::WIDGET_STATUS status)
  {
    ...
    if (status == FGUI::WIDGET_STATUS::HOVERED || m_bIsOpened)
      FGUI::RENDER.Rectangle((arWidgetRegion.m_iLeft + 2), (arWidgetRegion.m_iTop + 2), (arWidgetRegion.m_iRight - 4), (arWidgetRegion.m_iBottom - 4), {35, 35, 35});
    else
      FGUI::RENDER.Rectangle((arWidgetRegion.m_iLeft + 2), (arWidgetRegion.m_iTop + 2), (arWidgetRegion.m_iRight - 4), (arWidgetRegion.m_iBottom - 4), {30, 30, 30});
    ...
    FGUI::RENDER.Outline(arWidgetRegion.m_iLeft + 1, arWidgetRegion.m_iTop + 1, arWidgetRegion.m_iRight - 2, arWidgetRegion.m_iBottom - 2, { 22, 22, 22 });
    ...
    FGUI::RENDER.Text(arWidgetRegion.m_iLeft + (dmTitleTextSize.m_iWidth + 20), arWidgetRegion.m_iTop + (arWidgetRegion.m_iBottom / 2) - (dmTitleTextSize.m_iHeight / 2), m_anyFont, {255, 255, 255}, m_prgpEntries.first[m_ullSelectedEntry]);
    ...
    if (m_bIsOpened)
    {
      if (m_prgpEntries.first.size() > 14)
      {
        ...
        {
          FGUI::RENDER.Rectangle((arScrollBarRegion.m_iLeft + 3), static_cast<int>((arScrollBarRegion.m_iTop + flCalculatedPosition) + 5), dmScrollBarThumb.m_iWidth, dmScrollBarThumb.m_iHeight, {220, 223, 231});
        }
        else
        {
          FGUI::RENDER.Rectangle((arScrollBarRegion.m_iLeft + 3), static_cast<int>((arScrollBarRegion.m_iTop + flCalculatedPosition) + 5), dmScrollBarThumb.m_iWidth, static_cast<int>(flCalculatedSize - 10), {220, 223, 231});
        }
      }
      else
      {
        // dropdown list body
        FGUI::RENDER.Outline(arWidgetRegion.m_iLeft, (arWidgetRegion.m_iTop + 21), arWidgetRegion.m_iRight, (m_prgpEntries.first.size() * m_iEntrySpacing), {40, 40, 40});
        FGUI::RENDER.Outline(arWidgetRegion.m_iLeft + 1, (arWidgetRegion.m_iTop + 21) + 1, arWidgetRegion.m_iRight - 2, (m_prgpEntries.first.size() * m_iEntrySpacing) - 2, {22, 22, 22});
        FGUI::RENDER.Rectangle((arWidgetRegion.m_iLeft + 2), (arWidgetRegion.m_iTop + 21) + 2, (arWidgetRegion.m_iRight - 4), (m_prgpEntries.first.size() * m_iEntrySpacing) - 4, {30, 30, 30});
        ...
        for (std::size_t i = 0; i < m_prgpEntries.first.size(); i++)
        {
          ...
          // check if the user is hovering/have selected an entry
          if (FGUI::INPUT.IsCursorInArea(arEntryRegion) || m_ullSelectedEntry == i)
            FGUI::RENDER.Rectangle(arEntryRegion.m_iLeft + 2, arEntryRegion.m_iTop + 2, arEntryRegion.m_iRight - 4, arEntryRegion.m_iBottom - 2, {35, 35, 35});

          FGUI::RENDER.Text(arEntryRegion.m_iLeft + 5, (arEntryRegion.m_iTop + (arEntryRegion.m_iBottom / 2) - (FGUI::RENDER.GetTextSize(m_anyFont, m_prgpEntries.first[i]).m_iHeight / 2)), m_anyFont, { 255, 255, 255 }, m_prgpEntries.first[i]);
        }
      }
    }
    ...
    {
        FGUI::RENDER.Rectangle((arWidgetRegion.m_iLeft + arWidgetRegion.m_iRight - 10) - 8, arWidgetRegion.m_iTop + ((arWidgetRegion.m_iBottom / 2) - 3) + 4, 8, 1, { 20, 20, 20 });
        FGUI::RENDER.Rectangle((arWidgetRegion.m_iLeft + arWidgetRegion.m_iRight - 10) - 7, arWidgetRegion.m_iTop + ((arWidgetRegion.m_iBottom / 2) - 3) + 3, 6, 1, { 20, 20, 20 });
        FGUI::RENDER.Rectangle((arWidgetRegion.m_iLeft + arWidgetRegion.m_iRight - 10) - 6, arWidgetRegion.m_iTop + ((arWidgetRegion.m_iBottom / 2) - 3) + 2, 4, 1, { 20, 20, 20 });
        FGUI::RENDER.Rectangle((arWidgetRegion.m_iLeft + arWidgetRegion.m_iRight - 10) - 5, arWidgetRegion.m_iTop + ((arWidgetRegion.m_iBottom / 2) - 3) + 1, 2, 1, { 20, 20, 20 });
    }
  }
    ...
    if (m_prgpEntries.first.size() > 14)
    {
      if (m_bIsDraggingThumb)
      {
        ...
        if (FGUI::INPUT.IsKeyHeld(MOUSE_1))
        {
          ...
          {
            ptCursorPos.m_iY -= static_cast<int>(flHeightDelta);
          }

          // new vertical position ratio
          float flNewYRatio = static_cast<float>(ptCursorPos.m_iY) / static_cast<float>(m_dmSize.m_iHeight);
          m_iScrollThumbPosition = static_cast<int>(flNewYRatio * m_prgpEntries.first.size());
